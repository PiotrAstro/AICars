...
class PPO:
    ...
    def train_categorical_crossentropy(self, states, actions_indexes, rewards, expected_rewards=None, epochs_actor=10, epochs_critic=10):
        ...
        states = torch.from_numpy(states).float().to(self.device)
        expected_rewards = torch.from_numpy(expected_rewards).float().to(self.device)
        expected_rewards = expected_rewards.view(-1, 1)

        # Convert actions to one-hot encoding
        actions_indexes = torch.from_numpy(actions_indexes).long().to(self.device)
        actions_onehot = F.one_hot(actions_indexes, self.action_dim).float()

        dataset = TensorDataset(states, actions_onehot, expected_rewards)
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True)
        for _ in range(epochs_actor):
            for batch_states, batch_actions_onehot, _ in dataloader:
                self.actor_optimizer_categorical.zero_grad()
                action_probabilities, _ = self.model(batch_states)
                loss = self.actor_categorical_loss(action_probabilities, batch_actions_onehot)
                loss.backward()
                self.actor_optimizer_categorical.step()

        for _ in range(epochs_critic):
            for batch_states, _, batch_expected_rewards in dataloader:
                self.critic_optimizer.zero_grad()
                _, values = self.model(batch_states)
                loss = self.critic_loss(values, batch_expected_rewards)
                loss.backward()
